...
class BaseTagset:
    ...
    def add(self, key: Optional[str] = None, value: Optional[str] = None, tag: Optional[BaseTag] = None):
        """
            添加标签。如果键已存在，则覆盖。
    ...
    def remove(self, key: Optional[str] = None, value: Optional[str] = None, tag: Optional[BaseTag] = None):
        """
            删除标签。
    ...
    def export(self):
        return {
            tag.key: tag.value for tag in self.book.values()
        }
    ...
    def get(
        ...
        for t_key, t_value in self.book.items():
            key_match = (key is None or t_key.lower() == key.lower()) if case_insensitive else (key is None or t_key == key)
            value_match = (value is None or t_value.value.lower() == value.lower()) if case_insensitive else (value is None or t_value.value == value)
