...
def test_class_from_str():
	...
	result = datetime.strptime(datestamp_str, "%Y-%m-%d %H:%M:%S").replace(tzinfo = datestamp.TIME_ZONE)
	assert datestamp_obj.datestamp == result

def test_class_from_timestamp():
	timestamp = 1609459200
	datestamp_obj = datestamp.from_timestamp(timestamp)
	result = datetime.fromtimestamp(timestamp, datestamp.TIME_ZONE).replace(tzinfo = datestamp.TIME_ZONE)
	assert datestamp_obj.datestamp == result
...
def test_instance_set():
	...
	result = datetime.strptime(datestamp_str, "%Y-%m-%d %H:%M:%S").replace(tzinfo = pytz.timezone("Australia/Melbourne"))
	assert datestamp_obj.datestamp == result
...
def test_instance_get_date():
	datestamp_str = "2021-01-01 00:00:00"
	datestamp_obj = datestamp.from_str(datestamp_str = datestamp_str)
	assert datestamp_obj.get_date() == datetime.strptime("2021-01-01", "%Y-%m-%d").date()
...
def get_datestamp():
	datestamp_str = "2021-01-01 00:00:00"
	datestamp_obj = datestamp.from_str(datestamp_str = datestamp_str)
	assert datestamp_obj.get_datestamp() == datetime.strptime("2021-01-01 00:00:00", "%Y-%m-%d %H:%M:%S")
